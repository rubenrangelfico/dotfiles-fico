...
& $PythonPath -m pip install --quiet --upgrade `
    requests `
    websocket-client `
    keyring `
    win10toast 2>$null
...
'@ | Set-Content $TokensFile -Encoding UTF8
...
foreach ($xmlFile in $TaskXmls) {
    ...
    # Deregister existing task (ignore errors)
    Unregister-ScheduledTask -TaskName $taskName -Confirm:$false -ErrorAction SilentlyContinue

    # Load XML and register
    try {
...
Write-Host "     -> Set YOUR_TEAM_ID and YOUR_CHANNEL_ID"
